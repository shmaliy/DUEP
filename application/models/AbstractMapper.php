...
class Application_Model_AbstractMapper
{
    ...
    /**
     * Create and return empty row model
     * 
     * @return Application_Model_Abstract object
     */
    public function model()
    {
    	$name = $this->_formatModelName(get_class($this));
    	
    	$fieldKeys = $this->getDbTable()->info(Zend_Db_Table_Abstract::COLS);
    	$fields = array_flip($fieldKeys);
    	$fields = array_fill_keys($fieldKeys, null);
    	return new $name(array('fields' => $fields));
    }
	...
	{
		// If model not provided - create default
		if (empty($model)) {
			$model = $this->model();
		}
		
		// If provided and invalid - error
		if (!$model instanceof Application_Model_Abstract) {
			throw new Exception("Invalid data model provided for find operation", 500);
		}
	 ...
	 * @return mixed
	 */
	public function fetchRow($where = null, $order = null, $model = null)
	{
		// If model not provided - create default
		if (empty($model)) {
			$model = $this->model();
		...
		// If provided and invalid object - error
		if (!$model instanceof Application_Model_Abstract) {
			throw new Exception("Invalid data model provided for find operation", 500);
		}
